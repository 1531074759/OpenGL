...
public class MultisampleConfigChooser implements GLSurfaceView.EGLConfigChooser {
    static private final String TAG = MultisampleConfigChooser.class.getSimpleName();
    ...
    public EGLConfig chooseConfig(EGL10 egl, EGLDisplay display) {
        ...
        if (numConfigs <= 0) {
            if (LoggerConfig.ON) {
                Log.v(
                    TAG,
                    "No normal multisampling config was found. Attempting to create a coverage multisampling configuration, for the nVidia Tegra2. See the EGL_NV_coverage_sample documentation.");
            }
            ...
            if (numConfigs <= 0) {
                if (LoggerConfig.ON) {
                    Log.v(TAG,
                        "Could not find a multisample config. Trying without multisampling...");
                }
